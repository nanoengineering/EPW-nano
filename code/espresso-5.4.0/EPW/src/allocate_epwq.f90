...
#include "f_defs.h"
  ...
  USE elph2,        ONLY : elph, el_ph_mat, el_imp_mat
...
! THL: FFT dimensions dffts (smooth grid) should be replaced by dfftp (dense grid)
  ALLOCATE (dmuxc ( dfftp%tg_nnr, nspin, nspin))    
  !
  ALLOCATE (eigqts ( nat))    
...
! SP : from new PHonon/PH/allocate_phq.f90 
  ...
  if (eimp_mode == 5 .or. eimp_mode == 6) &
     allocate (el_imp_mat(nbnd, nbnd, nks))
  !
