...
#include "f_defs.h"
  ...
  USE epwcom,      ONLY: nbndsub, nwanxx, proj, iprint, dis_win_min, &
                         ...
                         spinors, wdata, dis_num_iter, num_cg_steps, trial_step, guiding_centres
  !
  implicit none
  !
  integer :: i
  ...
  WRITE (iuwinfil, '("dis_num_iter ", i7)')   dis_num_iter
  WRITE (iuwinfil, '("num_cg_steps ", i7)')   num_cg_steps
  WRITE (iuwinfil, '("trial_step ", f9.3)')   trial_step
  IF (guiding_centres) WRITE (iuwinfil,'(a)') "guiding_centres=.true."
  !
