  ...
  ! THL-
  ! wannier
  INTEGER           :: dis_num_iter, num_cg_steps
  REAL(KIND=DP)     :: trial_step
  LOGICAL           :: guiding_centres
  ! save time or memory
  LOGICAL           :: save_m_mat, save_m_matw, save_m_ph, save_t_el
  ! other
  INTEGER           :: bte, run, nkfdos1, nkfdos2, nkfdos3, epdim, nepdope, ndopexx=50
  ...
  LOGICAL           :: edos_read, asr_eph, ph_read, ephl_read, fscheck, ifc_read, elop, &
                       phdrag, shengbte_read, alloy_read, alloy_pot ! for phonon drag and alloy-phon scattering
  REAL(KIND=DP)     :: frac_type(2) ! Qian, for alloy-elec scattering, limited to binary alloy
  CHARACTER(LEN=10) :: smearing, vg_el, vg_ph, nptype
  REAL(KIND=DP), DIMENSION(50) :: epdope
  ! phonon-electron
  INTEGER           :: nqf1_ph, nqf2_ph, nqf3_ph, nkf1_ph, nkf2_ph, nkf3_ph
  ! THL---
  !
  ...
  ! eimp_mode : controls the way to calculate electron-impurity scattering
  !
  ...
  REAL (KIND=DP) :: degaussw, fsthick, wmin, wmax, ecutse, dis_win_min, dis_win_max, &
       ...
       max_memlt, fermi_energy, wmin_specfun, wmax_specfun, dielec, imp_charge, &
       defectdens
  ...
  logical :: phinterp , elinterp, tphases, epstrict, tshuffle2, elecselfen, phonselfen, &
       ...
       efermi_read, system_2d, delta_approx, bte_o, eimpbread, eimpbwrite, &
  ...
  CHARACTER(len=80) :: defectname
  REAL (kind=DP) :: L_D(25, 25)
  ...
  ! ep/eimp_check
  logical :: epcheck
  integer :: nbnd_c, nbnd_v
  character(len=1) :: xk_c, xk_v
