...
!, note
         !no matter what always set rec_code_read to be 10
!         rec_code_read = 10
         !
      END IF
      !
...
!note,
          !no matter what always set done_irr to be .false.
!          done_irr(irr) = .false.
       ENDIF
    ENDIF
    IF (trans) THEN
...
!
           ...
           DO irr=0,irr_iq(iq)
              ...
              ENDIF
!note
           !no matter what always set done_irr_iq to be .false.
!              done_irr_iq(irr,iq) = .false.
           END DO
...
!
        ...
        write(stdout,*) ' check done_iq in ph_restart:', done_iq
