...
SUBROUTINE elphon()
  ...
  IMPLICIT NONE
  !
  INTEGER :: irr, imode0, ipert, is, npe, i
  ...
  REAL(DP) :: celldm_(6), rydcm1
  CHARACTER(LEN=3) :: atm(ntyp)
  
  write(stdout,*)
  write(stdout,*) ' start ElPhon calculation..'
  ...
  !
  ! output phonon frequency
  !
  open(unit = 111, file='freq_q.dat')
  rydcm1 = 13.6058d0 * 8065.5d0
!  write(stdout,*)
!  write(stdout,*) ' phonon freq check:'
  do i = 1, 3*nat
     write(111,*) ' #mode',i,':',sqrt(abs(w2(i)))*rydcm1
!     write(stdout,*) '   #mode:',i, sqrt(abs(w2(i)))*rydcm1
  enddo
  close(111)
  !
...
SUBROUTINE elphel (irr, npe, imode0, dvscfins)
  ...
  write(stdout,*)
  write(stdout,*) '  Start elphel-matrix calculation ..'
  write(stdout,*) '  ####-------------------------------------------####'
  write(stdout,*) '  ####  <\psi(k+q)|dV_{SCF}/du^q_{i a}|\psi(k)>  ####'
  write(stdout,*) '  ####-------------------------------------------####'
  !
...
SUBROUTINE elphsum ( )
  ...
  USE control_ph,  ONLY : tmp_dir_phq, xmldyn, current_iq, &
                          eph_bndmin, eph_bndmax, eph_xk, trans, &
                          nks_eph, xk_eph
  ...
  INTEGER :: ik, ikk, ikq, isig, ibnd, jbnd, ipert, jpert, nu, mu, &
       vu, ngauss1, nsig, iuelph, ios, i,k,j, ii, jj, ik_k, ik_eph
  ...
  logical :: find_xk
  integer :: band_gamma, band_x, ikBZ, isym0
  real(DP) :: unorm
  complex(DP) :: elphmat_mode(nbnd,nbnd,3*nat), elphmat0(nbnd,nbnd,3*nat,3*nat)
  ...
  ! ADD start here =========================
  ! output e-ph matrix element on modes ====
  nkBZ  = nk1*nk2*nk3
  ...
  write(stdout,*)
  write(stdout,*) ' Check elphmat in phonon mode representation :'
  write(stdout,*)
  write(stdout,*) ' nks  =',nks
  write(stdout,*) ' nksq =',nksq

  write(stdout,*) ' check the 1st symmetry operation (should be identity):'
  write(stdout,'(3(1x,i5))') s(1,1:3,1)
  write(stdout,'(3(1x,i5))') s(2,1:3,1)
  write(stdout,'(3(1x,i5))') s(3,1:3,1)

  isym0 = 1
!NOTE
  if (.not.(trans)) then
     !
     open(unit=22222, file='eph_matrix.dat')
!     open(unit=22223, file='eph_matrix_polar.dat')
!     open(unit=22224, file='eph_matrix_deform.dat')
!     open(unit=22225, file='eph_matrix_lr.dat')
!     open(unit=22226, file='eph_matrix_sr.dat')
!     open(unit=22227, file='eph_matrix_scf.dat')
     write(22222,'(3(1x,i6))') nks_eph, eph_bndmax-eph_bndmin+1, nmodes
!     write(22223,'(3(1x,i6))') nks_eph, eph_bndmax-eph_bndmin+1, nmodes
!     write(22224,'(3(1x,i6))') nks_eph, eph_bndmax-eph_bndmin+1, nmodes
!     write(22225,'(3(1x,i6))') nks_eph, eph_bndmax-eph_bndmin+1, nmodes
!     write(22226,'(3(1x,i6))') nks_eph, eph_bndmax-eph_bndmin+1, nmodes
!     write(22227,'(3(1x,i6))') nks_eph, eph_bndmax-eph_bndmin+1, nmodes
  !  do ik = 1, nks
     do ik = 1, nksq
...
!        if ((xk(1,ik_k)==0).and.(xk(2,ik_k)==0).and.(xk(3,ik_k)==0)) find_gamma
!        = .true.
        if (.not.(eph_xk)) then
           if (((xk(1,ik_k)==0).and.(abs(xk(2,ik_k)-1)<1e-5).and.(xk(3,ik_k)==0)) .or. &
               ((xk(2,ik_k)==0).and.(abs(xk(3,ik_k)-1)<1e-5).and.(xk(1,ik_k)==0)) .or. &
               ((xk(3,ik_k)==0).and.(abs(xk(1,ik_k)-1)<1e-5).and.(xk(2,ik_k)==0))) then
              find_xk = .true.
           endif
           write(stdout,*) ' xk =', xk(1:3,ik_k)
        else
           do ik_eph = 1, nks_eph
              if ( (xk(1,ik_k) == xk_eph(1,ik_eph)) .and. &
                   (xk(2,ik_k) == xk_eph(2,ik_eph)) .and. &
                   (xk(3,ik_k) == xk_eph(3,ik_eph)) ) then
                 find_xk = .true.
              endif
           enddo
        endif
        ...
        if (find_xk) then
           ...
           write(stdout,*)
           write(stdout,*) ' find k-point.. at #pool', my_pool_id
           write(stdout,*) ' ik =',ik
           write(stdout,*) ' xk =',xk(1:3,ik_k)
           write(stdout,*) ' ene =',et(eph_bndmin:eph_bndmax,ik_k)

           itemp2 = 0
           if (.not.(eph_xk)) then
              do ikBZ = 1, nkBZ
                 if (eqBZ(ikBZ) == ik) then
                    itemp2 = sBZ(ikBZ)
                    exit
                 endif
              enddo
           ...
           !
           if (itemp2==0) then
              write(stdout,*) ' eqBZ numbering wrong.'
           elseif (itemp2/=1) then
              write(stdout,*) ' sBZ chosen ==>', itemp2
           endif
           write(stdout,*)
           !
           do ibnd = eph_bndmin, eph_bndmax
           do jbnd = eph_bndmin, eph_bndmax
              !
              do ii = 1, nmodes
                 do jj = 1, nmodes
                    elphmat0(jbnd,ibnd,ii,jj) = conjg(el_ph_mat(jbnd,ibnd,ik,ii))* el_ph_mat(jbnd,ibnd,ik,jj)
!                    elphmat1(jbnd,ibnd,ii,jj) = conjg(el_ph_mat_c(jbnd,ibnd,ik,ii))* el_ph_mat_c(jbnd,ibnd,ik,jj)
!                    elphmat2(jbnd,ibnd,ii,jj) = conjg(el_ph_mat_s(jbnd,ibnd,ik,ii))* (el_ph_mat_s(jbnd,ibnd,ik,jj))
!                    elphmat3(jbnd,ibnd,ii,jj) = conjg(el_ph_mat_lr(jbnd,ibnd,ik,ii))* (el_ph_mat_lr(jbnd,ibnd,ik,jj))                     
!                    elphmat4(jbnd,ibnd,ii,jj) = conjg(el_ph_mat_sr(jbnd,ibnd,ik,ii))* (el_ph_mat_sr(jbnd,ibnd,ik,jj))
!                    elphmat5(jbnd,ibnd,ii,jj) = conjg(el_ph_mat_scf(jbnd,ibnd,ik,ii))* (el_ph_mat_scf(jbnd,ibnd,ik,jj))
                 enddo
              enddo
              !
              write(stdout,*) '    elphmat on pattern without symmetrization:'
              write(stdout,*) '    --- (jbnd, ibnd) =', jbnd, ibnd

              do nu = 1, nmodes
                 write(stdout,*) '       imode =',nu
                 write(stdout,*) '       elphmat =', sqrt(abs(elphmat0(jbnd,ibnd,nu,nu)))
 !                write(stdout,*) '       elphmat_c =', sqrt(abs(elphmat1(jbnd,ibnd,nu,nu)))
 !                write(stdout,*) '       elphmat_s =', sqrt(abs(elphmat2(jbnd,ibnd,nu,nu)))
              enddo
              write(stdout,*) ' --------------------'
              call symm ( elphmat0(jbnd,ibnd,:,:), u, xq, s, itemp2, rtau, irt, at, bg, nat)
!              call symm ( elphmat1(jbnd,ibnd,:,:), u, xq, s, itemp2, rtau, irt, at, bg, nat)
!              call symm ( elphmat2(jbnd,ibnd,:,:), u, xq, s, itemp2, rtau, irt, at, bg, nat)
!              call symm ( elphmat3(jbnd,ibnd,:,:), u, xq, s, itemp2, rtau, irt, at, bg, nat)
!              call symm ( elphmat4(jbnd,ibnd,:,:), u, xq, s, itemp2, rtau, irt, at, bg, nat)
!              call symm ( elphmat5(jbnd,ibnd,:,:), u, xq, s, itemp2, rtau, irt, at, bg, nat)
           enddo
           enddo
           !
           !
           ...
           write(stdout,*)
           write(stdout,*) '  elphmat on pattern after symmetrization:'
           write(stdout,*) '  ------ (jbnd, ibnd) =', jbnd, ibnd
           do nu = 1, nmodes
              write(stdout,*) '    imode =',nu
              write(stdout,*) '    elphmat =', sqrt(abs(elphmat0(jbnd,ibnd,nu,nu)))
 ...
 !             write(stdout,*) '    elphmat_s =', sqrt(abs(elphmat2(jbnd,ibnd,nu,nu)))
   ...
   !        write(stdout,*) '  dyn:'
   !        do ii = 1, nmodes
   !           write(stdout,'(12(1x,f14.7))') dyn(ii,1:nmodes)
   !        enddo
           !
           write(stdout,*)
           elphmat_mode = 0.d0
 !          elphmat_modec = 0.d0
 !          elphmat_modes = 0.d0
 !          elphmat_mode_lr = 0.d0
 !          elphmat_mode_sr = 0.d0
 !          elphmat_mode_scf = 0.d0

           do ibnd = eph_bndmin, eph_bndmax
           do jbnd = eph_bndmin, eph_bndmax

           write(stdout,*)
           write(stdout,*) '  elphmat on mode:'
           write(stdout,*) '  ------ (jbnd, ibnd) =', jbnd, ibnd

           do nu = 1, nmodes
              unorm = 0.d0
              do mu = 1, nmodes
                 unorm = unorm + dyn (mu, nu) * CONJG(dyn (mu, nu) )
              enddo
              write(stdout,*) '   mode# =', nu
              write(stdout,*) '        unorm=', sqrt(unorm)
              write(stdout,*) '        freq =', sqrt(abs(w2(nu)))* 13.6058d0 * 8065.5d0
              write(stdout,*) (dyn (mu,nu)/sqrt(unorm), mu=1,3*nat)
              !
              do ii = 1, nmodes
                 do jj = 1, nmodes
                    elphmat_mode(jbnd,ibnd,nu) = elphmat_mode(jbnd,ibnd,nu) + DBLE(conjg(dyn(ii,nu)) * &
                        elphmat0(jbnd,ibnd,ii,jj) * dyn(jj,nu)) / unorm
!                    elphmat_modec(jbnd,ibnd,nu) = elphmat_modec(jbnd,ibnd,nu) + DBLE(conjg(dyn(ii,nu)) * &
!                        elphmat1(jbnd,ibnd,ii,jj) * dyn(jj,nu)) / unorm
!                    elphmat_modes(jbnd,ibnd,nu) = elphmat_modes(jbnd,ibnd,nu) + DBLE(conjg(dyn(ii,nu)) * &
!                        elphmat2(jbnd,ibnd,ii,jj) * dyn(jj,nu)) / unorm
!                    elphmat_mode_lr(jbnd,ibnd,nu) = elphmat_mode_lr(jbnd,ibnd,nu) + DBLE(conjg(dyn(ii,nu)) * &
!                        elphmat3(jbnd,ibnd,ii,jj) * dyn(jj,nu)) / unorm
!                    elphmat_mode_sr(jbnd,ibnd,nu) = elphmat_mode_sr(jbnd,ibnd,nu) + DBLE(conjg(dyn(ii,nu)) * &
!                        elphmat4(jbnd,ibnd,ii,jj) * dyn(jj,nu)) / unorm
!                    elphmat_mode_scf(jbnd,ibnd,nu) = elphmat_mode_scf(jbnd,ibnd,nu) + DBLE(conjg(dyn(ii,nu)) * &
!                        elphmat5(jbnd,ibnd,ii,jj) * dyn(jj,nu)) / unorm
                 enddo
              enddo
   !           if ((ibnd==5).and.(jbnd==5)) then
                 write(stdout,*) '    imode =',nu,jbnd,ibnd
                 write(stdout,*) '    elphmat =', sqrt(abs(elphmat_mode(jbnd,ibnd,nu)))
!                 write(stdout,*) '    elphmat_c =', sqrt(abs(elphmat_modec(jbnd,ibnd,nu)))
!                 write(stdout,*) '    elphmat_s =', sqrt(abs(elphmat_modes(jbnd,ibnd,nu)))
!                 write(stdout,*) '    elphmat_lr =', sqrt(abs(elphmat_mode_lr(jbnd,ibnd,nu)))
!                 write(stdout,*) '    elphmat_sr =', sqrt(abs(elphmat_mode_sr(jbnd,ibnd,nu)))
!                 write(stdout,*) '    elphmat_scf =', sqrt(abs(elphmat_mode_scf(jbnd,ibnd,nu)))

                 write(22222, '(4(i6,1x),E16.9)') ik, jbnd, ibnd, nu, sqrt(abs(elphmat_mode(jbnd,ibnd,nu)))
!                 write(22223, '(4(i6,1x),E16.9)') ik, jbnd, ibnd, nu, sqrt(abs(elphmat_modec(jbnd,ibnd,nu)))
!                 write(22224, '(4(i6,1x),E16.9)') ik, jbnd, ibnd, nu, sqrt(abs(elphmat_modes(jbnd,ibnd,nu)))
!                 write(22225, '(4(i6,1x),E16.9)') ik, jbnd, ibnd, nu, sqrt(abs(elphmat_mode_lr(jbnd,ibnd,nu)))
!                 write(22226, '(4(i6,1x),E16.9)') ik, jbnd, ibnd, nu, sqrt(abs(elphmat_mode_sr(jbnd,ibnd,nu)))
!                 write(22227, '(4(i6,1x),E16.9)') ik, jbnd, ibnd, nu, sqrt(abs(elphmat_mode_scf(jbnd,ibnd,nu)))
   !           endif
           enddo
