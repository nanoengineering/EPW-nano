...
SUBROUTINE openfilq()
  ...
  CHARACTER (len=256) :: filint, fildvscf_rot, fildvscf_lr
...
!!!!!!!!!!!!!!!!!!!!!!!! END OF ACFDT TEST !!!!!!!!!!!!!!!!
  ...
  if (lrange_eph) then
     CALL open_buffer (21000, 'bar_lr', lrbar, io_level, exst_mem, exst, tmp_dir)
  endif
...
400 IF (trim(fildvscf).NE.' ') THEN
     ...
     IF ( me_bgrp == 0 ) THEN
        ...
        if (lrange_eph) then
           fildvscf_lr = trim(fildvscf) // '_lr'
           fildvscf_lr = trim(adjustl(fildvscf_lr))
           write(stdout,*) ' fildvscf_lr =',fildvscf_lr
           CALL diropn (27000, fildvscf_lr, lrdrho, exst)
        endif
