...
SUBROUTINE xsf_fast_datagrid_3d &
  ...
  write(stdout,*) ' inside xsf_fast_datagrid_3d...'
  write(stdout,*) ' nr1..3:', nr1,nr2,nr3
  write(stdout,*) ' nr1..3x:', nr1x,nr2x,nr3x
  ...
  WRITE(ounit,'(3f15.9)') (BOHR_RADIUS_ANGS*alat*at(i,1),i=1,3) ! in ANGSTROMS
  ! 2nd spanning (=lattice) vector
  WRITE(ounit,'(3f15.9)') (BOHR_RADIUS_ANGS*alat*at(i,2),i=1,3)
  ! 3rd spanning (=lattice) vector
  WRITE(ounit,'(3f15.9)') (BOHR_RADIUS_ANGS*alat*at(i,3),i=1,3)
  ...
  DO i3=0,nr3
     ...
     ENDDO
  ENDDO
  WRITE(ounit,'(6e18.10:)') (rho(ind_x(i),ind_y(i),ind_z(i)),i=1,count)
...
SUBROUTINE xsf_datagrid_3d &
  ...
  write(stdout,*) ' inside xsf_datagrid_3d...'
  write(stdout,*) ' nx,ny,nz :', nx,ny,nz
  write(stdout,*) ' e1 :', e1
  write(stdout,*) ' e2 :', e2
  write(stdout,*) ' e3 :', e3
  ...
  WRITE(ounit,'(3f10.6)') (BOHR_RADIUS_ANGS*alat*x0(i),i=1,3) ! in ANGSTROMS
  ! 1st spanning (=lattice) vector
  WRITE(ounit,'(3f15.9)') (BOHR_RADIUS_ANGS*alat*e1(i)*m1,i=1,3) ! in ANGSTROMS
  ! 2nd spanning (=lattice) vector
  WRITE(ounit,'(3f15.9)') (BOHR_RADIUS_ANGS*alat*e2(i)*m2,i=1,3) ! in ANGSTROMS
  ! 3rd spanning (=lattice) vector
  WRITE(ounit,'(3f15.9)') (BOHR_RADIUS_ANGS*alat*e3(i)*m3,i=1,3)
     ...
     DO iy=1,ny
        DO ix=1,nx
           IF (count<6) THEN
              count = count + 1
           ELSE
              WRITE(ounit,'(6e18.10)') (rho(ind_x(i),ind_y(i),ind_z(i)),i=1,6)
              count=1
           ENDIF
           ind_x(count) = ix
           ind_y(count) = iy
