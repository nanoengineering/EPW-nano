...
9020 FORMAT(/'          k =',3(1x,F10.7),'     band energies (ev):'/ )
9021 FORMAT(/'          k =',3(1x,F10.7),' (',I6,' PWs)   bands (ev):'/ )
9030 FORMAT( '  ',8F10.5 )
...
SUBROUTINE get_homo_lumo ( ehomo, elumo )
  ...
  ! ... formats
  !
9043 FORMAT(/'     highest occupied level (ev): ',F10.5 )
9042 FORMAT(/'     highest occupied, lowest unoccupied level (ev): ',2F10.5 )
9041 FORMAT(/'     the spin up/dw Fermi energies are ',2F10.5,' ev' )
9040 FORMAT(/'     the Fermi energy is ',F10.5,' ev' )
  !
